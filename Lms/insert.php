...
$fire = mysqli_query($conn,$sql);
 ...
 if(mysqli_num_rows($fire) > 0)
 ...
 else if(mysqli_num_rows($checkemail) > 0)
   {
    ...
    {
		$query ="INSERT INTO users (FirstName,LastName,Email,UserName,Role,ContactNumber,Password,Address,Designation,Picture) VALUES ('{$firstName}', '{$lastName}' , '{$email}', '{$userName}','{$role}','{$ContactNbr}', '{$hash_Pswd}', '{$Address}', '{$Designation}', '{$file}' )";
